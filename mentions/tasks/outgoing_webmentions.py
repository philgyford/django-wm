...
try:
    ...
    # Replace missing @shared_task import with a wrapper that does nothing
    shared_task = lambda func, *args, **kwargs: lambda *args, **kwargs: func(
        *args, **kwargs
    )
    pass
...
def process_outgoing_webmentions(source_url: str, text: str) -> int:
    ...
    for link_url in links_in_text:
        ...
        try:
            ...
            _save_status(
                outgoing_status, STATUS_MESSAGE_TARGET_UNREACHABLE, success=False
            )
            continue

        if response.status_code >= 300:
            log.warning(f'Link "{link_url}" returned status={response.status_code}')
            _save_status(
                outgoing_status,
